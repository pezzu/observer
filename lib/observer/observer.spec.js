...
test('Results of browser and storage are passed to comparator', async () => {
  ...
  mockBrowser.load.mockImplementation(job => Promise.resolve(dataBrowser));
  mockStorage.load.mockImplementation(job => Promise.resolve(dataStorage));
...
test('Compare, notify aren\'t called if either browser or storage are failed to load', async () => { 
